...
class LoginWEB(tornado.web.RequestHandler):
    def get(self):
        ...
        if (username != "" and password != ""):
            name = tools.searchDB('admin', ['username'], where="username = '" + username + "'")
            email = tools.searchDB('admin', ['email'], where="username = '" + username + "'")
            if name!= () or email!=():
                if username == name[0][0].decode('utf-8') or username == email[0][0]:
                    password_0 = tools.searchDB('admin', ['password'], where="password = '" + password + "'")
                    password0 = ''
                    if password_0!=():
    ...
    def get(self):
        ...
        if(username!="" and password1!= "" and password2!="" and email!=""):
            ...
            if name != username:
                ...
                lens = tools.searchDB('admin',['count(id)'])[0][0]
                if password1==password2 :
                    listOfvalues = [str(lens+1),username,password1,password2]
                    tools.insertDB('admin',listOfColumns,listOfvalues)
